...
// otherwise the raw bytes of the string are hashed via a deterministic XOR
// spread and the result becomes the trace_id. A fresh SpanID is always
// generated via rand. This ensures deterministic conversion so every log
// line in every service carries the same trace_id when the caller supplies
...
func normaliseRequestID(id string) string {
	...
	// XOR-spread over 16 bytes — goal is determinism and non-zero output,
